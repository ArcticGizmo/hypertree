...
- A **spatial map** is a second *model* of the same desktops, swapped to with **`Tab`** (or **Settings → Appearance → Map model**). Instead of a vertical stack of rows, desktops become **rooms** you place anywhere on a 2-D grid, and branches become logical, stable-coloured **groups**. Move a room (**`Ctrl+Arrow`** or drag), its contiguous **block** (**`Ctrl+Shift+Arrow`** or **`⇧`-drag**), or a whole group (select it with **`g`**); recolour groups from the **`Shift+G`** panel; **`t`** tidies drifted groups back together (reuniting scattered pieces without disturbing their shape), **`Ctrl+Z`** undoes it. Deleting a room leaves its spot empty rather than reshuffling. It's the same arrangement seen a different way — swapping changes nothing about your desktops. (Design notes: [docs/design/spatial-map-plan.md](docs/design/spatial-map-plan.md).)
