...
/// <summary>
/// Covers persistence + restore: groups survive a restart, and desktops that no longer exist are
...
public class PersistenceTests
{
    ...
    [Fact]
    public void A_group_survives_a_restart_when_its_desktops_still_exist()
    {
        var store = new InMemoryStore();
        // OS lists the top desktops AND the group's desktops.
        var ids = new[] { D(0), D(1), D(10), D(11) };

        var m1 = new NavigationModel(new FakeDesktopController(ids, 0), store);
        m1.AddGroup(G("feat", (10, "a"), (11, "b")));

        // Restart: fresh model, same store, same OS desktops.
        var m2 = new NavigationModel(new FakeDesktopController(ids, 0), store);
        Assert.Equal(1, m2.GroupCount);
        NavMap map = m2.BuildMap();
        Assert.Equal(2, map.TopRow.Count);              // D(10)/D(11) are NOT orphaned into the top row
        Assert.Equal("feat", map.Groups[0].Name);
        Assert.Equal(new[] { "a", "b" }, map.Groups[0].Desktops.Select(t => t.Label));
    }

    [Fact]
    public void Vanished_group_desktops_are_dropped_on_restore()
    {
        var store = new InMemoryStore();
        new NavigationModel(new FakeDesktopController(new[] { D(0), D(10), D(11) }, 0), store)
            .AddGroup(G("feat", (10, "a"), (11, "b")));

        // Restart with those group desktops gone from the OS entirely.
        var m = new NavigationModel(new FakeDesktopController(new[] { D(0), D(1) }, 0), store);
        Assert.Equal(0, m.GroupCount); // whole group dropped — nothing to orphan
    }

    [Fact]
    public void Partially_surviving_group_keeps_only_the_live_desktops()
    {
        var store = new InMemoryStore();
        new NavigationModel(new FakeDesktopController(new[] { D(0), D(10), D(11) }, 0), store)
            .AddGroup(G("feat", (10, "a"), (11, "b")));
        ...
        Assert.Single(m.BuildMap().Groups[0].Desktops);
    }
}
