<#
.SYNOPSIS
    Installs Hypertree on Windows.

.DESCRIPTION
    Downloads the Velopack installer (Hypertree-win-Setup.exe) for a GitHub release, verifies it against
    the SHA256SUMS.txt published alongside it, and runs it. Everything after this - every subsequent
    update - is Velopack's job: right-click the tray icon -> "Check for updates".
...
.PARAMETER Version
    Install a specific version (e.g. 0.3.4) instead of the latest release. A leading "v" is fine.
    Also reads $env:HYPERTREE_VERSION, which is how you pin through the piped one-liner:
        $env:HYPERTREE_VERSION = '0.3.4'; irm .../install.ps1 | iex

.PARAMETER Repo
    owner/name of the GitHub repository to install from. Defaults to ArcticGizmo/hypertree; override for
    a fork. Also reads $env:HYPERTREE_REPO.
...
.EXAMPLE
    ...
    & ([scriptblock]::Create((irm https://raw.githubusercontent.com/ArcticGizmo/hypertree/main/install.ps1))) -Version 0.3.4
#>
#Requires -Version 5.1
param(
    [string] $Version = $env:HYPERTREE_VERSION,
    [string] $Repo    = $(if ($env:HYPERTREE_REPO) { $env:HYPERTREE_REPO } else { 'ArcticGizmo/hypertree' })
)
...
function Install-Hypertree {
    [CmdletBinding()]
    param([string] $Version, [string] $Repo)

    $ErrorActionPreference = 'Stop'

    $SetupAsset = 'Hypertree-win-Setup.exe'
    $SumsAsset  = 'SHA256SUMS.txt'

    # --- Preflight -------------------------------------------------------------------------------------
    if ([System.Environment]::OSVersion.Platform -ne 'Win32NT') {
        throw 'Hypertree drives Windows virtual desktops and only installs on Windows.'
    }
    if (-not [System.Environment]::Is64BitOperatingSystem) {
        throw 'Hypertree ships as 64-bit only; this looks like a 32-bit Windows.'
    }
    if ($env:PROCESSOR_ARCHITECTURE -eq 'ARM64') {
        Write-Warning 'ARM64 Windows detected. Hypertree is x64-only, so it will run under emulation.'
    }
    ...
    # --- Resolve the release ---------------------------------------------------------------------------
    $release = Get-HypertreeRelease -Repo $Repo -Version $Version
    $tag     = $release.tag_name
    Write-Host "Installing Hypertree $tag" -ForegroundColor Cyan
    ...
    $work = Join-Path ([System.IO.Path]::GetTempPath()) ("hypertree-install-" + [guid]::NewGuid().ToString('N').Substring(0, 12))
...
"@
        ...
        # --- Install -----------------------------------------------------------------------------------
        # Velopack's installer needs no admin rights: it installs to %LocalAppData%\Hypertree, registers the
        # uninstaller and Start Menu shortcut, and - the part that matters here - launches the tray app
        # before exiting. The tray then puts its own install directory on the per-user PATH, which is what
        # makes `htree` resolve in a new terminal.
        #
        # So do NOT use `Start-Process -Wait`: that waits for the started process *and all its descendants*,
        # which includes the tray app we just installed. Since Hypertree is meant to keep running, the wait
        # never returns and the one-liner hangs forever. Waiting on the Setup process's own handle instead
        # returns as soon as the install itself is finished.
        ...
        if (-not $proc.WaitForExit(10 * 60 * 1000)) {
            $keepWork = $true
            Write-Warning "The installer is still running after 10 minutes - leaving it to finish on its own. Delete $work once Hypertree has installed."
            return
        }
        if ($proc.ExitCode -ne 0) {
            throw "The Hypertree installer exited with code $($proc.ExitCode)."
        }
    }
    finally {
    ...
    Write-Host ''
    Write-Host "Hypertree $tag is installed and starting in the system tray." -ForegroundColor Green
    Write-Host '  Ctrl+Alt+M opens the map; Ctrl+Alt+Arrow navigates the desktop tree.' -ForegroundColor DarkGray
    Write-Host '  Left-click the tray icon for the command palette, right-click for the menu.' -ForegroundColor DarkGray
    Write-Host '  The install folder goes on your PATH: open a NEW terminal, then run  htree status' -ForegroundColor DarkGray
    Write-Host '  Updates from here on are in-app: right-click the tray icon -> Check for updates.' -ForegroundColor DarkGray
}
...
function Get-HypertreeRelease {
    param([string] $Repo, [string] $Version)

    $headers = @{ 'Accept' = 'application/vnd.github+json'; 'User-Agent' = 'hypertree-install.ps1' }
...
function Save-File {
    ...
    $req.UserAgent = 'hypertree-install.ps1'
    $req.Timeout = 60000
...
Install-Hypertree -Version $Version -Repo $Repo
