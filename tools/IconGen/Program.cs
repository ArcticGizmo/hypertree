...
// Generates every raster icon asset from the single source-of-truth SVG (hypertree.svg).
//
//   src/Hypertree.App/Assets/icon.png   256x256 PNG — window icons and the in-app logo
//   src/Hypertree.App/Assets/icon.ico   multi-resolution ICO (16..256) — tray icon + the .exe ApplicationIcon
//   landing-icon.png                    512x512 PNG — the README header logo
//
// Re-run after editing hypertree.svg:  dotnet run --project tools/IconGen
// (or tools/gen-icons.ps1, which also restores packages.)
// Windows-only: the SVG is rendered through System.Drawing, which only runs on Windows.
...
string svgPath = Path.Combine(repoRoot, "hypertree.svg");
...
// Inkscape exports width/height in physical units (e.g. "256mm"); the Svg renderer then applies a
// mm->px viewport scale (~3.78x at 96 DPI) on top of our own transform, throwing the artwork off the
// canvas. Pin the document size to its viewBox user units so the viewport maps 1:1 and our crop/scale
// math (all in viewBox units) is the only transform in play. No-op for unitless (px) SVGs like perch's.
...
// The artwork doesn't necessarily fill its viewBox — there's transparent margin. To make the icon use
// as much of its pixel canvas as possible (it matters most in the tiny tray), we crop to the actual
// drawn content, re-center it in a square, and scale that to fill the frame. PAD keeps a sliver of
// breathing room so anti-aliased edges don't clip.
...
string assetsDir = Path.Combine(repoRoot, "src", "Hypertree.App", "Assets");
...
// Walks up from the tool's binary location to the directory that holds hypertree.svg.
static string FindRepoRoot(string start)
{
    var dir = new DirectoryInfo(start);
    while (dir != null)
    {
        if (File.Exists(Path.Combine(dir.FullName, "hypertree.svg")))
            return dir.FullName;
        dir = dir.Parent;
    }
