...
.DESCRIPTION
    ...
    Hypertree.Core), so its logic is exercised here instead. Run it after touching install.ps1:
...
$src = (Get-Content $installPs1 -Raw) -replace '(?m)^Install-Hypertree\s+.*$', ''
...
$pipeline = 'install.ps1', 'publish.bat', 'run.bat', 'tools\test-install.ps1',
'tools\gen-icons.ps1', 'tools\gen-icons.cmd', '.github\workflows\release.yml'
...
$sums = @"
$h  Hypertree-win-Setup.exe
$('b' * 64)  Hypertree-win-Portable.zip
$('c' * 64) *Hypertree-0.3.4-full.nupkg
"@
Check 'finds the setup entry' ((Get-ExpectedHash -Sums $sums -Name 'Hypertree-win-Setup.exe' -Tag v1) -eq $h.ToUpperInvariant())
Check 'finds a binary-mode (*) entry' ((Get-ExpectedHash -Sums $sums -Name 'Hypertree-0.3.4-full.nupkg' -Tag v1) -eq ('c' * 64).ToUpperInvariant())
Check 'does not prefix-match a different asset' ((Get-ExpectedHash -Sums $sums -Name 'Hypertree-win-Portable.zip' -Tag v1) -eq ('b' * 64).ToUpperInvariant())
Check 'handles CRLF manifests' ((Get-ExpectedHash -Sums "$h  Hypertree-win-Setup.exe`r`n" -Name 'Hypertree-win-Setup.exe' -Tag v1) -eq $h.ToUpperInvariant())
...
CheckThrows 'throws on an empty manifest' { Get-ExpectedHash -Sums '' -Name 'Hypertree-win-Setup.exe' -Tag 'v1' } 'no entry for'
CheckThrows 'throws on an HTML error page as the manifest' { Get-ExpectedHash -Sums '<html>404</html>' -Name 'Hypertree-win-Setup.exe' -Tag 'v1' } 'no entry for'
...
$asString = "$h  Hypertree-win-Setup.exe`n"
$asBytes = [System.Text.Encoding]::UTF8.GetBytes($asString)
Check 'an already-decoded string parses' ((Get-ExpectedHash -Sums (Decode $asString) -Name 'Hypertree-win-Setup.exe' -Tag v1) -eq $h.ToUpperInvariant())
Check 'byte[] content (what GitHub actually returns) parses the same' ((Get-ExpectedHash -Sums (Decode $asBytes) -Name 'Hypertree-win-Setup.exe' -Tag v1) -eq $h.ToUpperInvariant())
...
$rel = [pscustomobject]@{
    ...
    assets   = @([pscustomobject]@{ name = 'Hypertree-win-Setup.exe'; browser_download_url = 'https://example/setup' })
}
Check 'resolves a present asset' ((Get-AssetUrl -Release $rel -Name 'Hypertree-win-Setup.exe') -eq 'https://example/setup')
...
if (-not $payload) {
    ...
    $tmp = Join-Path $env:TEMP ("hypertree-test-" + [guid]::NewGuid().ToString('N').Substring(0, 8))
...
$localSetup = Join-Path $root 'releases\Hypertree-win-Setup.exe'
if (-not (Test-Path -LiteralPath $localSums) -or -not (Test-Path -LiteralPath $localSetup)) {
    Write-Host '  SKIP  no local SHA256SUMS.txt + Hypertree-win-Setup.exe - run publish.bat to cover this.' -ForegroundColor Yellow
}
else {
    $want = Get-ExpectedHash -Sums (Get-Content -LiteralPath $localSums -Raw) -Name 'Hypertree-win-Setup.exe' -Tag 'local'
