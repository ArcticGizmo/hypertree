...
- **Edit a recipe's steps** — open a recipe from the manager to edit each step's target, **arguments**, and **working directory**. This is what makes location-keyed apps restore usefully: give a VS Code step its project folder (as an argument — VS Code is a singleton per folder), or a terminal step its working directory, so they come back on the right thing instead of blank.
