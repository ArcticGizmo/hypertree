...
- **Variables** — a command can contain `{name}` tokens (e.g. `code {dir}`, `wt -d {repo}`); the builder lists them so you can give each a default and mark folders, and **applying** a loadout prompts you to fill them first. So one loadout outfits any project. A built-in `{dir}` is recognised (folder-typed); auto-filling it from the current directory via an `htree` CLI verb is still to come.
