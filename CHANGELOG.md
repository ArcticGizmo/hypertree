...
- **Recipes — build a workspace by hand** — the command palette's **Recipes…** manager opens a graphical builder where a branch is drawn as **desktop rows**, each split into a slot per **monitor**, and each slot holds an ordered list of **commands** (typed naturally — `code C:\proj`, `wt -d "C:\proj"`, with an optional name and working directory). Add and remove desktops, add/edit/remove commands, and save it as a named, reusable recipe in `recipes.json`. Recipes key their desktops by *label*, so they're portable and reboot-proof. (Design: [docs/design/session-restore.md](docs/design/session-restore.md).)
- **Apply a recipe as a new branch** — **Apply recipe…** rebuilds the workspace: it creates the recipe's desktops plus a temporary **staging** desktop, runs every command there, moves each launched window to the desktop and **monitor** it belongs on, then folds them into a new branch and lands you in it. A **live progress overlay** shows each command as it launches and is placed, and can be cancelled. Apps already open (single-instance) are reported rather than duplicated; anything launched but not placed can be closed or left, and is only ever closed while still on the staging desktop. (Monitor placement is best-effort "right screen"; exact position/size comes later.)
