...
- **Loadouts — build a workspace by hand** — the command palette's **Loadouts…** manager opens a graphical builder where a branch is drawn as **desktop rows**, each split into a slot per **monitor**, and each slot holds an ordered list of **commands** (typed naturally — `code C:\proj`, `wt -d "C:\proj"`, with an optional name and working directory). Add and remove desktops, add/edit/remove commands, and save it as a named, reusable loadout in `loadouts.json`. Loadouts key their desktops by *label*, so they're portable and reboot-proof. (Design: [docs/design/session-restore.md](docs/design/session-restore.md).)
- **Variables** — a command can contain `{name}` tokens (e.g. `code {dir}`, `wt -d {repo}`); the builder lists them so you can give each a default and mark folders, and **applying** a loadout prompts you to fill them first. So one loadout outfits any project. A **folder** variable's fill field offers a type-ahead of matching directories and a **Browse…** button.
- **Apply from a chord or the CLI** — **`Ctrl+Alt+E`** opens the apply-a-loadout picker (rebindable in Settings). And **`htree populate <name>`** applies a loadout from the shell, sending your **current directory** as the built-in `{dir}` variable — so `cd` into a repo (or a git worktree) and `htree populate dev` builds the whole workspace around it. Other variables pass as `--name=value`; the tray fills the rest from their defaults and prompts for anything still missing.
- **Apply a loadout as a new branch** — **Apply loadout…** builds the workspace: it creates the loadout's desktops plus a temporary **staging** desktop, runs every command there, moves each launched window to the desktop and **monitor** it belongs on, then folds them into a new branch and lands you in it. A **live progress overlay** shows each command as it launches and is placed, and can be cancelled. Apps already open (single-instance) are reported rather than duplicated; anything launched but not placed can be closed or left, and is only ever closed while still on the staging desktop. (Monitor placement is best-effort "right screen"; exact position/size comes later.)
