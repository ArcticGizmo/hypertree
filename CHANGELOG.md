...
- **Variables** — a command can contain `{name}` tokens (e.g. `code {dir}`, `wt -d {repo}`); the builder lists them so you can give each a default and mark folders, and **applying** a loadout prompts you to fill them first. So one loadout outfits any project.
