...
- **Saving is now a review** — snapshotting a branch builds a draft recipe and opens it as a **prefilled list of suggestions**, one per captured window, each with the app path, its monitor, and the window title as a hint. You then refine each into a real command — target, **arguments**, **working directory**, **monitor** — or trim it. This is what makes location-keyed apps restore usefully: give a VS Code step its project folder as an argument (VS Code is a singleton per folder), or a terminal step its working directory, instead of coming back blank.
- **Monitor placement** — a recipe records which monitor each window was on, and restore puts it back on that screen (best-effort — right monitor, re-maximised if it was maximised; exact position/size comes later). The monitor is captured automatically and editable per step.
